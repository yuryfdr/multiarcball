  ...
  }
  {
    if ( Fl::event_button() == FL_LEFT_MOUSE || Fl::event_button1()){
      ...
      if ( event == FL_PUSH || event == FL_RELEASE || event == FL_DRAG ){
        if (Fl::event_state(FL_SHIFT) && Fl::event_state(FL_CTRL))  handle_dolly(mvct::XY((double)Fl::event_x()/w(),(double)Fl::event_y()/h()),FL_PUSH==event,FL_RELEASE==event);
        else if (Fl::event_state(FL_SHIFT)) handle_pan(mvct::XY((double)Fl::event_x()/w(),(double)Fl::event_y()/h()),FL_PUSH==event,FL_RELEASE==event);
